...
class FathomStatsController extends Controller
{
     ...
     * @return array
     */
    public function __invoke(Request $request, $siteId, $days = 7)
    {
        $key = 'fathomStats_'.$siteId.'_'.$days;
        if (Cache::has($key)) {
            ...
            'statBlocks' => [
                ...
                [
                    ...
                    'prevStat' => $statData['pageviews'] > 1000 ? round($statData['pageviews'] / 1000,
                        2).'k' : $statData['pageviews'],
                    'diff' => abs($pageviews_diff) > 1000 ? round(abs($pageviews_diff) / 1000,
                        2).'k' : abs($pageviews_diff),
                    'up' => $pageviews_diff > 0,
                ],
                [
                    'title' => 'avg duration',
                    'mainStat' => sprintf('%02d:%02d', ($avg_duration_seconds / 60), $avg_duration_seconds % 60),
                    'prevStat' => sprintf('%02d:%02d', ($prev_avg_duration_seconds / 60),
                        $prev_avg_duration_seconds % 60),
