<template>
  <Card class="flex flex-col items-center justify-center">
    ...
    <div class="flex w-full items-center justify-between px-3 py-3">
        ...
        <div v-if="timePeriod !== null">
          <div class="relative flex" :class="$attrs.class">
            <select
              ...
              @change="updateStats"
              :class="['form-control-xs form-select-bordered', selectClasses]"
            >
            ...
            <IconArrow class="form-select-arrow pointer-events-none" />
          </div>
        </div>
      ...
      <div>
        <a v-if="fathomLink !== null" :href="fathomLink">Go to Fathom</a>
        <Loader v-else class="text-gray-300" width="30" />
      </div>
    </div>
  </Card>
...
const siteId = ref(props.card?.entityId || null);
...
function updateStats() {
  Nova.request()
    .get(
      "/nova-vendor/fathom-stats-display/getStats/" +
        siteId.value +
        "/" +
        timePeriod.value
    )
